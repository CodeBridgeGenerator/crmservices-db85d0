...
const statusArray = ["Draft","Sent","Signed","Active","Expired","Terminated"];
...
const [accountId, setAccountId] = useState([])
    ...
    useEffect(() => {
        ...
        if (!_.isEmpty(props?.entity)) {
            init = initilization({ ...props?.entity, ...init }, [opportunityId,accountId], setError);
        }
        set_entity({...init});
        setError({});
    }, [props.show]);
    ...
    const validate = () => {
        ...
        const error = {};
            ...
            if (_.isEmpty(_entity?.contractNumber)) {
                error["contractNumber"] = `Contract Number field is required`;
                ret = false;
            }
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            opportunityId: _entity?.opportunityId?._id,accountId: _entity?.accountId?._id,contractNumber: _entity?.contractNumber,status: _entity?.status,startDate: _entity?.startDate,endDate: _entity?.endDate,contractValue: _entity?.contractValue,signedDate: _entity?.signedDate,terms: _entity?.terms,fileUrl: _entity?.fileUrl,
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <Dropdown id="accountId" value={_entity?.accountId?._id} optionLabel="name" optionValue="value" options={accountIdOptions} onChange={(e) => setValByKey("accountId", {_id : e.value})}  />
            </span>
            <small className="p-error">
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <InputText id="contractNumber" className="w-full mb-3 p-inputtext-sm" value={_entity?.contractNumber} onChange={(e) => setValByKey("contractNumber", e.target.value)}  required  />
            </span>
            <small className="p-error">
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <InputNumber id="contractValue" className="w-full mb-3 p-inputtext-sm" value={_entity?.contractValue} onChange={(e) => setValByKey("contractValue", e.value)}  />
            </span>
            <small className="p-error">
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <InputTextarea id="terms" rows={5} cols={30} value={_entity?.terms} onChange={ (e) => setValByKey("terms", e.target.value)} autoResize  />
            </span>
            <small className="p-error">
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <InputText id="fileUrl" className="w-full mb-3 p-inputtext-sm" value={_entity?.fileUrl} onChange={(e) => setValByKey("fileUrl", e.target.value)}  />
            </span>
            <small className="p-error">
