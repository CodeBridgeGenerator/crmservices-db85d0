...
import contractsFakerFactory from "./contractsFakerFactory";
...
const ContractsFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [opportunityIdIds, setopportunityIdIds] = useState();
...
client.service("accounts")
    ...
    const onRun = async () => {
        let fakeData = contractsFakerFactory(props.user,fakerCount,opportunityIdIds,accountIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("contracts").create(elem));
...
export default connect(mapState, mapDispatch)(ContractsFakerDialogComponent);
