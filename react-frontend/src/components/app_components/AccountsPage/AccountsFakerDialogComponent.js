...
import accountsFakerFactory from "./accountsFakerFactory";
...
const AccountsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = accountsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("accounts").create(elem));
...
export default connect(mapState, mapDispatch)(AccountsFakerDialogComponent);
