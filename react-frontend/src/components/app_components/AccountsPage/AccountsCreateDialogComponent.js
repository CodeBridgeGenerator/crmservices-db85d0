...
const AccountsCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            name: _entity?.name,industry: _entity?.industry,website: _entity?.website,email: _entity?.email,phone: _entity?.phone,address: _entity?.address,status: _entity?.status,notes: _entity?.notes,
        ...
        const result = await client.service("accounts").create(_data);
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Accounts created successfully" });
        props.onCreateResult(result);
        } catch (error) {
            console.debug("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in Accounts" });
        }
        setLoading(false);
    };
    ...
    return (
        <Dialog header="Create Accounts" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="accounts-create-dialog-component">
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <InputText id="industry" className="w-full mb-3 p-inputtext-sm" value={_entity?.industry} onChange={(e) => setValByKey("industry", e.target.value)}  />
            </span>
            <small className="p-error">
            {!_.isEmpty(error["industry"]) ? (
              <p className="m-0" key="error-industry">
                {error["industry"]}
              </p>
            ) : null}
          </small>
...
export default connect(mapState, mapDispatch)(AccountsCreateDialogComponent);
