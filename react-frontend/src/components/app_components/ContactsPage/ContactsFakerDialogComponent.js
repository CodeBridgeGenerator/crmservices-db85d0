...
import contactsFakerFactory from "./contactsFakerFactory";
...
const ContactsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = contactsFakerFactory(props.user,fakerCount,accountIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("contacts").create(elem));
...
export default connect(mapState, mapDispatch)(ContactsFakerDialogComponent);
