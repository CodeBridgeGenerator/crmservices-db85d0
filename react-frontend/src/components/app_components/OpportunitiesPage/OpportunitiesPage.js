...
const OpportunitiesPage = (props) => {
  ...
  useEffect(() => {
    ...
    client
      ...
      .find({
        query: {
          $limit: 10000,
          accountId: urlParams.singleAccountsId,
          primaryContactId: urlParams.singleContactsId,
          $populate: [
            {
              path: "createdBy",
              service: "users",
  ...
  const deleteAll = async () => {
    ...
    const promises = data.map((e) =>
      client.service("opportunities").remove(e._id),
    );
    await Promise.all(
      promises.map((p) =>
        p.catch((error) => {
