...
import opportunitiesFakerFactory from "./opportunitiesFakerFactory";
...
const OpportunitiesFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [accountIdIds, setaccountIdIds] = useState();
const [primaryContactIdIds, setprimaryContactIdIds] = useState();
    ...
    useEffect(() => {
        ...
        client.service("accounts")
                        ...
                        .then((data) => {
                        setaccountIdIds(data.data);
                        });
client.service("contacts")
                        .find({ query: { $select: ["_id","firstName","lastName"] } })
                        .then((data) => {
                        setprimaryContactIdIds(data.data);
                        });
    ...
    const onRun = async () => {
        let fakeData = opportunitiesFakerFactory(props.user,fakerCount,accountIdIds,primaryContactIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("opportunities").create(elem));
...
export default connect(mapState, mapDispatch)(OpportunitiesFakerDialogComponent);
