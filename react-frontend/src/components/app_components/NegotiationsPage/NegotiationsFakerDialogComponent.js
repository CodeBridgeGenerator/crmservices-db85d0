...
import negotiationsFakerFactory from "./negotiationsFakerFactory";
...
const NegotiationsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = negotiationsFakerFactory(props.user,fakerCount,opportunityIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("negotiations").create(elem));
...
export default connect(mapState, mapDispatch)(NegotiationsFakerDialogComponent);
