...
const NegotiationsPage = (props) => {
  ...
  useEffect(() => {
    ...
    client
      ...
      .find({
        query: {
          $limit: 10000,
          opportunityId: urlParams.singleOpportunitiesId,
          $populate: [
            {
              path: "createdBy",
              service: "users",
      ...
      .catch((error) => {
        ...
        props.alert({
          title: "Negotiations",
          type: "error",
          message: error.message || "Failed get Negotiations",
        });
      });
  ...
  const deleteAll = async () => {
    ...
    const promises = data.map((e) =>
      client.service("negotiations").remove(e._id),
    );
    await Promise.all(
      promises.map((p) =>
        p.catch((error) => {
